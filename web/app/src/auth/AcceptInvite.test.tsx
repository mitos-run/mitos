...
describe('AcceptInvite page (pre-auth)', () => {
  ...
  it('shows an honest expired message and no CTAs when the lookup reports expired, before the visitor ever signs in', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        lookupResponse({ org_name: 'Acme', inviter_name: 'Alice', email_hint: 'jo***@example.com', role: 'member', state: 'expired' }),
      ),
    )
    render(<AcceptInvite token="tok-5" authenticated={false} />)
    await waitFor(() => expect(screen.getByText(/ask the person who invited you to send a new one/i)).toBeInTheDocument())
    expect(screen.queryByRole('link', { name: /sign in to accept/i })).not.toBeInTheDocument()
    expect(screen.queryByRole('link', { name: /create an account/i })).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /^join/i })).not.toBeInTheDocument()
  })

  it('shows an honest revoked message and no CTAs when the lookup reports revoked, before the visitor ever signs in', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        lookupResponse({ org_name: 'Acme', inviter_name: 'Alice', email_hint: 'jo***@example.com', role: 'member', state: 'revoked' }),
      ),
    )
    render(<AcceptInvite token="tok-6" authenticated={false} />)
    await waitFor(() => expect(screen.getByText(/ask whoever invited you for a new one/i)).toBeInTheDocument())
    expect(screen.queryByRole('link', { name: /sign in to accept/i })).not.toBeInTheDocument()
    expect(screen.queryByRole('link', { name: /create an account/i })).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /^join/i })).not.toBeInTheDocument()
  })
...
describe('AcceptInvite page (post-auth)', () => {
  ...
  it('reports the revoked message (not a generic failure) when the accept call fails on an invite that was revoked in the meantime', async () => {
    const fetchMock = vi
      .fn()
      // initial lookup: still shows pending from this viewer's perspective
      .mockResolvedValueOnce(
      ...
      // accept call fails (revoked in the meantime)
      .mockResolvedValueOnce({ ok: false, status: 400, text: async () => JSON.stringify({ error: { cause: 'no longer pending' } }) })
      // re-lookup reveals it was revoked
      .mockResolvedValueOnce(
        lookupResponse({ org_name: 'Acme', inviter_name: 'Alice', email_hint: 'jo***@example.com', role: 'member', state: 'revoked' }),
      )
    vi.stubGlobal('fetch', fetchMock)

    render(<AcceptInvite token="tok-7" authenticated />)
    const joinBtn = await screen.findByRole('button', { name: /join acme/i })
    await userEvent.click(joinBtn)
