...
// focus moves into the dialog (the textarea) on open, matching InviteModal's
// pattern (a plain confirm dialog elsewhere in the app was flagged for
// skipping this; every new dialog does it right from the start).
...
export function FeedbackButton({ caps, route }: { caps: Capabilities; route: string }) {
  ...
  return (
    <>
      <button type="button" className="feedback-button" aria-label="Send feedback" onClick={() => setOpen(true)}>
        <SpeechBubbleIcon />
      </button>
      {open && <FeedbackDialog caps={caps} feedback={caps.feedback} route={route} onClose={() => setOpen(false)} />}
    </>
  )
}
...
function FeedbackDialog({
  ...
  function send() {
    ...
    if (feedback.channel === 'email') {
      const url = `mailto:${feedback.target}?subject=${encodeURIComponent('Mitos feedback')}&body=${encodeURIComponent(body)}`
      window.location.assign(url)
    } else {
      const title = trimmed.slice(0, 60) || 'Feedback'
      const url = `https://github.com/${feedback.target}/issues/new?title=${encodeURIComponent(title)}&body=${encodeURIComponent(body)}`
  ...
  return (
    ...
    >
      <div role="dialog" aria-modal="true" aria-labelledby="feedback-dialog-title" className="card modal">
