...
// TestListenAndServeClosesGracefully asserts that Close unblocks the Accept loop
// and makes ListenAndServe return nil (a clean shutdown, not a fatal error), so
// forkd can shut the proxy down on signal without treating it as a crash. This
// mirrors the dnsproxy Server.Shutdown parity.
func TestListenAndServeClosesGracefully(t *testing.T) {
	p := NewProxy(staticResolver{ip2id: map[string]string{}}, &fakeDialer{}, nil)

	errCh := make(chan error, 1)
	go func() { errCh <- p.ListenAndServe("127.0.0.1:0") }()

	// Give the goroutine a moment to bind the listener, then close it.
