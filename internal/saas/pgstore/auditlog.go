...
func (l *PgAuditLog) Record(ctx context.Context, ev console.AuditEvent) error {
	const q = `
        INSERT INTO audit_log (org_id, actor, action, target, detail, created_at)
        VALUES ($1, $2, $3, $4, $5, $6)`
	if _, err := l.pool.Exec(ctx, q, ev.OrgID, ev.ActorID, ev.Action, ev.Target, ev.Detail, ev.At); err != nil {
		return fmt.Errorf("record audit event: %w", err)
	}
	return nil
}

// List returns the org's events, most recent first; id breaks a created_at tie
// so same-instant events keep reverse insertion order, matching MemAuditLog.
// It only ever returns the named org's events and always returns a non-nil
// slice. Timestamps are normalized to UTC so the round trip is deterministic
// and store-equivalent (see timeVal).
func (l *PgAuditLog) List(ctx context.Context, orgID string) ([]console.AuditEvent, error) {
	const q = `
        SELECT org_id, actor, action, target, detail, created_at
        FROM audit_log WHERE org_id = $1 ORDER BY created_at DESC, id DESC`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&ev.OrgID, &ev.ActorID, &ev.Action, &ev.Target, &ev.Detail, &ev.At); err != nil {
