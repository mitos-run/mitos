...
| Guest agent - Rust (`guest/agent-rs`) | PID 1 in guest; the PRODUCTION DEFAULT (Phase D, #310). Baked as `/init` by default via `guest/rootfs/build.sh` and `kvm-test.yaml`. Serves ONLY gRPC on vsock port 53 (AgentGRPCPort); SP1.5 merged all host callers to gRPC. SECURITY-SENSITIVE: requires named human reviewer; see `docs/security-review-policy.md`. | nothing | forkd / husk stub (gRPC path, port 53 only) |
| Guest agent - Go (`guest/agent`) | PID 1 in guest when AGENT_IMPL=go; the fallback/opt-out while the Go agent has not yet been removed (Phase E). Serves BOTH gRPC (port 53) and the legacy JSON protocol (port 52). | nothing | forkd / husk stub treat its output as data only |
...
| Guest agent as attack surface (Rust, `guest/agent-rs`) | **partial (gRPC surface; production default since Phase D #310)** | The Rust agent is the production default: baked as `/init` by `guest/rootfs/build.sh` (AGENT_IMPL=rust, now the default) and by `kvm-test.yaml`. It serves ONLY gRPC on vsock port 53. SP1.5 is merged: all host callers (fork, daemon, husk, smoke binaries) speak gRPC. The full firecracker-test suite (exec-via-vsock, fork-correctness, ws-smoke, SDK example, etc.) now boots the Rust agent. No fuzzing of the gRPC surface yet. SECURITY-SENSITIVE: requires named human reviewer before any PR touching `guest/agent-rs/src/sys/`, `guest/agent-rs/src/fork/`, `guest/agent-rs/src/init/mod.rs`, or `guest/agent-rs/src/main.rs` is merged. See `hack/rust-agent-cutover.md`. |
| Guest agent as attack surface (Go, `guest/agent`) | **partial (fallback/opt-out; not yet removed, see Phase E)** | The Go agent is the AGENT_IMPL=go fallback. It speaks gRPC on vsock port 53 and the legacy JSON protocol on vsock port 52. A 10MB line-buffer cap exists. No fuzzing of the protocol yet. SECURITY-SENSITIVE: requires named human reviewer before any PR touching `guest/agent` is merged. |
| In-guest self-service socket (`MITOS_SOCKET`, issue #22) | **mitigated (in-guest only; no new host surface)** | The guest agent serves a small JSON protocol over a unix socket INSIDE the VM at `MITOS_SOCKET` (default `/run/mitos.sock`, `guest/agent/selfservice.go`, `internal/guestsock`) so the in-VM workload can read its OWN identity and budget (and, once issue #25 wires it, request a budget-gated fork). The socket is reachable only from inside the guest: it is a unix socket on the guest's own tmpfs, NOT exposed on the tenant-facing HTTP sandbox API and NOT bridged to the host vsock, so it adds NO new host boundary; a caller of it is already running inside the untrusted guest at the same privilege as any `exec` child (KVM/Firecracker and the unprivileged husk pod, section 0, bound it). The `info` response carries NAMES and budget NUMBERS only (sandbox id, claim, pool, workspace names; fork caps), assembled by a key-whitelisting handler from the delivered env; no secret VALUE ever crosses it (the handler reads only the `MITOS_*` identity/budget keys, never the secret keys also present in the guest env). The host advertises the path and the sandbox's own id via the existing best-effort env-delivery channel (`withSelfServiceEnv`, `internal/daemon/server.go`); a listen failure is non-fatal so exec/files over vsock are unaffected. The same `MaxMessageBytes` line cap applies. The fork verb is not yet enabled: it returns an LLM-legible not-enabled error naming the orchestrator escalation path until the budget ledger (issue #25) is wired. |
