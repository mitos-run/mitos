...
| CSRF state and open-redirect defense | mitigated (unit-tested) | CSRF state: before redirecting to the OIDC provider the proxy mints a signed CSRF state token (HMAC over a random nonce, domain-tagged, short TTL) and records it in the state codec cookie. On the `/auth/callback` return the `state` parameter is verified against the cookie with constant-time compare; a missing, tampered, or expired state is rejected 400 before the code exchange runs. Open-redirect defense: the `rd` (redirect) parameter in `/start?rd=<label>` is validated against the live route table; it must name a currently registered route label. An unknown label or any value that is not a single DNS label is rejected 400, so the auth origin cannot be used as an open redirector to an attacker-controlled host. The `cb` (callback path) parameter additionally rejects values beginning with `//` or containing a backslash or CRLF, preventing scheme-relative or header-injection redirects. `internal/preview/authorigin.go`. Unit-tested: state cookie round-trip; tampered state rejected; expired state rejected; unknown label in `rd` rejected; `//`-prefix and backslash in `cb` rejected. |
| Verified-email audience rule: unverified email rejected | mitigated (unit-tested) | The `allowedEmailDomains` audience selector in the authorization pipeline checks BOTH that the caller's email suffix matches AND that `email_verified == true` in the session identity. An unverified email is rejected 403 regardless of its domain, closing the account-takeover path where an attacker registers a matching email at a provider that does not enforce ownership before issuing tokens. `internal/preview/authz.go` (the `Authorize` function checks `identity.EmailVerified` before the domain suffix comparison). Unit-tested: unverified email with a matching domain is rejected; verified email with matching domain is accepted; verified email with non-matching domain is rejected. |
