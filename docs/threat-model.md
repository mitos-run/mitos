...
| Patched Firecracker VMM provenance (forkd, husk-stub) | mitigated | The forkd and husk-stub images ship a Mitos-patched `firecracker` (live-fork: memfd/MAP_SHARED guest memory + UFFD write-protect + vmstate-only snapshot + restore-side memfd share/export/WP-offer) instead of the stock upstream binary; the stock jailer is unchanged. The patched binary is NOT a general-CDN download: it is built reproducibly in the `mitos-run/firecracker` fork on branch `mitos/wp-offer-on-restore-v1.15.0` (off `mitos/vmstate-only-snapshot-v1.15.0`) via `.github/workflows/build-patched-fc.yml` (green run `https://github.com/mitos-run/firecracker/actions/runs/28895721279`) and published as a GitHub release asset (`mitos-fc-wp-on-restore-v1.15.0`). `hack/install-firecracker-patched.sh` fetches it from a SINGLE pinned URL and verifies it against a pinned sha256 (`cfed95979ddb656d53881694965da0fb88fb0a92990e1a6ec7eb31edc9954e22`, 5,226,728 bytes) BEFORE install AND re-verifies the on-disk binary after, failing the build CLOSED on any mismatch, so a compromised release or a network substitution cannot install a different VMM. It reports `firecracker --version` = v1.15.0 (same as the stock jailer, so forkd's version-match check still passes) and is behaviour-identical to stock Firecracker v1.15.0 UNLESS `FIRECRACKER_MITOS_SHARED_MEM` or `FIRECRACKER_MITOS_WP_UDS` are set at runtime OR a `PUT /snapshot/create` sends `snapshot_type` `MitosVmstateOnly`; `Full` and `Diff` snapshots are byte-for-byte unchanged. This bump adds the m4b restore-side change (issue #832): when `FIRECRACKER_MITOS_SHARED_MEM` is set, the RESTORED source loads its guest RAM into a `MAP_SHARED` memfd (`memory::snapshot_file_memfd`) then runs the SAME `mitos_export_shared_memfd` + `mitos_wp_offer` the fresh-boot path already runs, so a restored source arms the live-cow fork instead of falling back to the Full snapshot. The change is entirely BEHIND the existing `FIRECRACKER_MITOS_SHARED_MEM` gate: the non-mitos restore path and `Full`/`Diff` snapshot restore stay byte-for-byte unchanged, so the runtime attack surface is UNCHANGED from stock until that gate is set. It adds no new syscall or network surface beyond the memfd + UFFD write-protect handshake already reviewed for the boot path (surface 3b), over the source VM's OWN guest RAM. The swap is a single COPY + RUN per image and is trivially revertable (drop it and the stock upstream firecracker from `hack/install-firecracker.sh` remains). Residual: the patched build does not yet carry cosign/SBOM attestation of its own (tracked with the image-provenance follow-up); trust rests on the pinned sha256 plus the reproducible fork build. |
