...
| Control | Status | Detail |
|---|---|---|
| controller ↔ forkd authn/authz (mTLS) | **open** | The gRPC service is not even registered yet (`internal/daemon/server.go:RegisterForkDaemonServer` is a TODO); the dial path uses `insecure.NewCredentials()` (`internal/controller/node_registry.go`). When wired, any pod that can reach node:9090 could request VMs. mTLS with SPIFFE-style identities or at minimum a cluster-CA client cert is required before the gRPC path ships. |
