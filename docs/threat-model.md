...
| Host resource exhaustion (memory + cpu + sandbox count + disk) | **mitigated (production-blocker #2)** | Four host-DoS dimensions are now capped, each as an O(1) admission/ceiling/sizing check OFF the warm-claim activate/fork hot path so they do not regress the warm-claim latency. (1) **Husk pod memory.** A husk pod previously carried a memory REQUEST only and no LIMIT, so a tenant VM could grow without bound and OOM the node. The controller now sets a memory LIMIT sized = request + headroom (`internal/controller/huskpod.go`, `huskMemoryLimit`), headroom = max(`--husk-memory-headroom` floor, default 256Mi; `--husk-memory-headroom-percent` of the request, default 25%). The headroom is load-bearing: the cgroup the limit caps holds the Firecracker VMM, the husk-stub, and CoW dirty-page slack ON TOP of the guest RAM, so a too-tight limit would OOM-kill a normally-running VM (which is why the limit must exceed the request); the headroom keeps the limit transparent to a legitimate VM while capping a runaway. The kubelet enforces the cgroup; the controller never throttles the running VM. Memory is left honest (no overcommit): Firecracker holds the guest RAM as genuinely resident pages, so overcommitting memory would OOM-kill live sandboxes under node memory pressure. (2) **Husk pod cpu.** The husk container now carries a CPU LIMIT equal to the pool's configured per-sandbox cpu (`spec.template.resources.cpu`, or 250m by default). The kubelet enforces this as `cgroup cpu.max`, bounding the CPU a single tenant sandbox can consume (a new CPU DoS cap per sandbox). The CPU REQUEST is set to a low floor (50m, `defaultHuskCPURequestFloor`) so idle warm husks pack densely onto nodes: the scheduler places pods by sum-of-requests, so the small request enables intentional node CPU overcommit while the limit caps each sandbox's burst. Restore and activate run up to the cpu limit (the configured per-sandbox cap, which operators set generously for burst workloads). CPU is burstable and safe to overcommit; memory is not, which is why the two dimensions are treated differently. QoS stays Burstable (requests less than limits for both cpu and memory); it does not regress to BestEffort. (3) **Per-node sandbox count.** The engine reported `MaxSandboxes` in `GetCapacity` but never enforced it at `Fork`, so a runaway tenant could exhaust a node by opening forks. `Engine.Fork` now refuses with the typed `ErrAtCapacity` once the live count reaches `--max-sandboxes` (`internal/fork/engine.go`, `admitFork`), BEFORE any verify, allocation, or Firecracker boot, mapped to gRPC `RESOURCE_EXHAUSTED` for the controller; 0 disables it. (4) **Concurrent streams per sandbox.** Capped via `--max-streams-per-sandbox` (see the Connect runtime exec/run_code and interactive Exec rows). (5) **Node local disk.** forkd's data dir (snapshots, CoW rootfs clones, volume backings, CAS) lives on finite node-local disk; placing builds and forks by memory alone fills it until the kubelet evicts forkd on DiskPressure (a hard crash, not a graceful "node full"). The capacity heartbeat now carries a disk-headroom signal: forkd reports the data-dir filesystem free and total bytes (a single `statfs` of the data dir on the heartbeat path, `internal/fork/capacity.go` seam, `disk_unix.go`), and the controller scheduler treats a node whose free fraction is below a fixed floor (15%, deliberately above the kubelet's default 10% nodefs.available hard-eviction threshold) as having NO build capacity, so placement backs off before eviction (`internal/controller/scheduler.go` `hasDiskHeadroom`, issue #465). A node reporting total 0 (statfs failed / dev / mock) is treated as an unknown, unlimited budget, mirroring the memory handling. Disk numbers are not secrets and are safe to log. This is the scheduler-side complement to the CAS LRU watermark eviction (issue #464, docs/failure-gc.md). Residuals: the 15% floor is a fixed internal constant, not yet operator-tunable, and disk is a backoff gate, not a packed dimension (the scheduler does not project a fork's marginal disk cost the way it projects memory). The memory headroom defaults are sized conservatively but are operator-tunable, not derived from a measured VMM+CoW profile per template (raise the floor if pods are OOM-killed at their configured RAM); the cpu request floor (50m) is a fixed internal knob, not operator-exposed (the cap is operator-visible via the pool template); the sandbox-count ceiling is per-node, not a global tenant quota; there is no per-tenant fair-share across sandboxes yet (a tenant with many sandboxes still consumes proportionally). | the memory headroom defaults are sized conservatively but are operator-tunable, not derived from a measured VMM+CoW profile per template (raise the floor if pods are OOM-killed at their configured RAM); the cpu request floor (50m) is a fixed internal knob, not operator-exposed (the cap is operator-visible via the pool template); the sandbox-count ceiling is per-node, not a global tenant quota; there is no per-tenant fair-share across sandboxes yet (a tenant with many sandboxes still consumes proportionally). |
| Hosted gateway quota + abuse + kill-switch enforcement (issue #341) | **mitigated (durable, replica-shared kill-switch when a database is configured; live concurrency cap enforced from cluster state; size and aggregate caps still open)** | The hosted gateway no longer admits every request: `cmd/gateway` wires the real `quota.Enforcer` (via `quota.GatewayAdapter`) in place of the `AllowAllQuota` stub, default-on for the hosted profile (`--enforce-quota`, overridable). On the `sandbox.create` path the enforcer checks in FAIL-CLOSED order: the kill-switch suspension store FIRST (a suspended org is refused before any tier or usage logic, and a store-read error is a DENY, never a silent allow), then tier resolution (an org resolves to the tier an OPERATOR granted it with the repeatable `--org-tier=<orgID>=<tier>` gateway flag, and to the free tier, the tightest caps, otherwise. The grant is operator configuration read at startup; no tenant input reaches it, the tier name is validated against the ladder so a typo aborts the process rather than resolving to a surprising tier at request time, a duplicate org is rejected so limits never depend on flag order, the match is exact so a near-miss org id gets the TIGHT tier, and every grant is logged at WARN at startup. Absence of a grant is never read as 'unlimited'. NOTE that the pro tier carries `Egress{Open: true}`, so granting it also opens that org's sandbox egress posture. Billing plans still do not drive tiers), then per-sandbox size, live concurrency and aggregate caps, and per-org plus per-IP rate buckets. The SAME suspension store is written by both the abuse-signal path and the billing suspender adapter (`quota/billing_adapter.go`), so an abuse flag or a billing `past_due`/spend-cap event suspends the org and the gateway then blocks it (proven end to end in `cmd/gateway` and `quota` tests). `ErrSuspended` maps to 403 naming the reason without leaking internal detail; an over-quota or rate denial maps to 429 with the limit and tier; no message leaks another org's data. The client IP for the per-IP buckets is resolved by `internal/saas/clientip` with the gateway's trusted-proxy-hops model (`RemoteAddr` unless `--trusted-proxy-hops > 0`; a caller-set leftmost `X-Forwarded-For` is never trusted and a too-short list fails closed to `RemoteAddr`), so a spoofed `X-Forwarded-For` cannot bypass per-IP limits (asserted in `gateway_clientip_test.go`). The org id is taken only from the gateway-verified request. The acceptable-use policy (`docs/legal/acceptable-use-policy.md`, a counsel-reviewable DRAFT scaffold) is the policy basis referenced from the kill-switch code path. Durability (issue #615): when the gateway has a database configured (the hosted profile; the chart injects the shared DSN), the kill-switch store is the Postgres `pgstore.PgSuspensionStore` (migration `0008_suspensions.sql`) behind `quota.CachedSuspensionStore`, so a suspension survives gateway restarts and binds EVERY replica; a suspension written on one replica takes effect on the others within the read-cache TTL (a few seconds), and a store-read error still fails CLOSED (deny). Without a database the store falls back to the in-process `MemSuspensionStore` (dev only; the startup log names the non-durable posture): suspensions then do not survive a restart and are not shared across replicas, so run a single replica or re-drive the signals. LIVE CONCURRENCY (issue #615 seam 2): the front-door `LiveUsage` source is the cluster-backed `controlplane.LiveCounter` whenever the gateway runs against the real control plane: it lists the org's Sandbox objects with the same tenancy scoping the control plane uses (org namespace plus org label, single-tenant mode included), counts every non-terminal phase, and counts `spec.replicas` per object (a fork fan-out of N is N running VMs, and the create path accepts a client-supplied replicas value, so counting objects would let a capped org fan out past its tier), so the per-tier concurrent-sandbox cap is enforced against what the org is ACTUALLY running; a live-count read error is a DENY (fail closed: an unreachable apiserver never reads as zero live sandboxes), and a startup self-check logs a persistent RBAC/scheme misconfiguration at ERROR with remediation so a fail-closed deny-all-creates posture is loud at boot, not discovered from customer reports. The zero-stub source remains only on the `--allow-stub` dev path, and the startup mode string names which caps are live. BILLING SUSPENSION now has two PRODUCTION paths into the SAME Postgres `suspensions` table the gateway reads (`cmd/console` `newBillingSuspender`): the drawdown cycle evaluates each active org's hard spend cap after settling its usage (`billing.Service.EnforceSpendCapFromLedger`, period spend = the calendar month's usage-drawdown debits), and the provider webhook drives the kill-switch on the transition into the suspended status, with fail-closed ordering (the suspension write precedes the status flip and a write failure is a 5xx so the provider retries, never a silent drop); previously the console built its billing service with NO suspender and the computed suspension went nowhere. Recovery is reason-scoped and payment-driven (issue #615): a cleared paid top-up lifts a spend_cap suspension (and the spend window resets at the payment, so the next drawdown cycle does not re-suspend for pre-payment spend), and an active-status transition lifts a dunning suspension (`quota.KillSwitch.LiftReason` via the `billing.SuspensionLifter` seam); the lift can NEVER touch a suspension under another reason (abuse, emergency stop, manual) or ANY suspension with a manual-review hold, which clear only through the operator Lift hook. The automated spend-cap suspend therefore carries NO manual hold (a held suspension could never be lifted by payment); holds remain the operator's tool. A suspension or lift reaches every gateway replica within the read-cache TTL. Residual: the per-sandbox SIZE caps are NOT enforced (the `GatewayAdapter.SizeOf` seam is unwired, so a create is checked with a zero spec and only the concurrency cap can trip; the control plane does NOT re-check), and the AGGREGATE resource caps are NOT enforced (sandboxes carry no per-create resource fields, honest aggregates need each sandbox's pool-resolved footprint, and the live counter deliberately reports zero for them rather than guessing), a create asking for replicas = N is admitted as +1 rather than +N (the body is not parsed at enforcement time; the fan-out lands in the next create's count), and check-then-create admission has no reservation step (a parallel burst can overshoot the cap by up to the tier's creation-rate burst); these are the deferred remainder of issue #615, together with the plan-backed tier resolver (an operator may now GRANT a tier with `--org-tier`, but nothing reads an org's BILLING PLAN, so a paying org is not automatically raised) and metered-overage inclusion in period spend (no invoice source pre-#618). Rate buckets, creation-rate cap, and kill-switch are unaffected. |
...
| `GET /v1/metering` on the husk pod `:9091` (usage collector source, issue #613) | **mitigated (unauthenticated by design, secret-free, read-only; org is the trusted controller-stamped label, never the pod's report; own-vm-id-only; cleartext-on-pod-network residual)** | In the husk topology every sandbox VM runs inside its own husk pod, which forkd's engine never tracks, so forkd's own `/v1/metering` reports nothing for it and `usage_records` stayed empty (hosted metered nothing). The husk stub now serves its ONE VM's CoW-aware metering report on `GET /v1/metering` on the same `:9091` mux (`cmd/husk-stub` `newSandboxMux`/`huskMeteringHandler`, `internal/husk` `Stub.Metering`), and the controller usage collector scrapes each claimed pod (`internal/usage/husksource.go`, `internal/controller/usage_scrape.go` `HuskPodScrapeLister`). The surface is bounded as follows. (1) UNAUTHENTICATED BY DESIGN: the route is mounted on the outer mux BEFORE the `/` catch-all so it is served OUTSIDE the per-sandbox bearer middleware, the same access class as forkd's identical `/v1/metering`, `/metrics`, and `/healthz` operational endpoints (a sandbox token grants no extra access and none is required); the exec/files/run_code routes on the same mux stay token-gated and fail closed exactly as the row above. (2) SECRET-FREE: the report carries only the vm-id and numeric byte/second counts, never argv, env, file bytes, or any token (asserted by `TestMeteringReportIsSecretFree`); memory bytes are reported as 0 pending the #613 fast-follow, so the report is a strict subset of forkd's. (3) SERVED FROM DORMANT: the server starts at pod startup so a warm-window scrape is a clean empty report (`{"Sandboxes":null}`), not a connection refusal; a bind failure fails the pod at startup (fail closed, never Ready). (4) UNTRUSTED-POD ORG TAGGING: the collector tags org SOLELY from the trusted `mitos.run/org` label the controller stamped from the per-org namespace (`HuskPodScrapeLister`), NEVER from anything the pod returns, and accepts ONLY the pod's OWN vm-id sample (`HuskSource.Collect` `orgOf`), so a compromised husk stub can bill only its own vm-id/org, never another tenant's (`TestHuskSourceIgnoresForeignVMID`). A pod with no trusted org label (self-host single-tenant) is omitted, never forced into an org. (5) ROBUSTNESS: a pod that is unreachable, non-200, or fails to decode is skipped-and-counted (`SkippedPods`), never failing the cycle for the healthy fleet. Residual: like forkd's node source the scrape is plaintext HTTP on the pod network and does not size-cap the response body (both are trusted-infra endpoints; shares the `:9091` in-cluster-TLS follow-up noted on the exec/files row), and the memory dimension under-meters until the #613 memory fast-follow lands (a revenue gap, no security impact). |
...
| `POST /v1/sandboxes/{id}/fork` (standalone live-state fork, issue #596) | **mitigated (path-injection guarded on BOTH ids, fail-closed reseed, idempotent; tokenless loopback on standalone, same as the rest of the server; hosted per-sandbox route is the org-scoped `sandbox.fork` op behind the same gateway auth/quota)** | The standalone sandbox-server forks an ALREADY-RUNNING sandbox: the source is the `{id}` path component and the child boots from a checkpoint of the paused source (live memory + a copy-on-write clone of the source's OWN writable rootfs) through `engine.ForkRunning` (`cmd/sandbox-server` `handleForkRunning`). It is bounded exactly like `handleFork`. (1) PATH-INJECTION: BOTH the source path id and the child body id must pass `safeIDComponent` (the same `^[a-zA-Z0-9][a-zA-Z0-9_-]{0,63}$` allowlist as the gRPC `validateSandboxID` and `handleFork`), rejected 400, so neither can traverse out of the `sandboxes/<id>` tree; a missing source is 404, never a template lookup. (2) FAIL-CLOSED reseed: a live fork restores a checkpoint, so the child shares the source's CRNG state until it reseeds; the handler runs the SAME `registerAndReseed` fail-closed handshake `handleFork` runs (an un-reseeded child is unregistered and reaped, never served). (3) IDEMPOTENT: a repeat under the same `Idempotency-Key` returns the first child, never a duplicate (issue #22). (4) AUTH: the standalone host listener is tokenless loopback by design (`AllowTokenless`, a single-tenant local server), the SAME trust model as `/v1/fork` and `/forward`, NOT a new weakening. FILESYSTEM CARRY: a live fork deliberately carries the source's on-disk filesystem into the child (this is the fix, matching the husk path); on-disk data the source itself wrote is user data by design, and PLATFORM secret inheritance into live forks is separately governed by the controller-level default-deny gate (fork-correctness row 3), unchanged here. HOSTED: the flat SDK reaches this route through the gateway, which maps `POST /v1/sandboxes/{id}/fork` to the dedicated `sandbox.fork` op (`internal/saas/gateway.go` `opFromPath`, issue #709), a TRUE live fork through the `FromSandbox` controller path; the org-scoped boundary for that op is its own section 7b row. The flat `POST /v1/fork` (no source in the path) stays a `sandbox.create` template claim. Proven by `cmd/sandbox-server` `TestLiveForkOfRunningSandboxReseedsAndSucceeds`/`TestLiveForkUnknownSourceReturns404`/`TestLiveForkRejectsUnsafeIDs`/`TestLiveForkIdempotencyKeyReturnsSameChild`, `internal/saas` `TestOpFromPathPerSandboxForkMapsToSandboxFork`, and end to end on KVM by `cmd/live-state-fork-smoke`. |
| `ANY /v1/sandboxes/{id}/expose/{port}/` (authenticated guest HTTP proxy, Mitos Expose slice 1, issues #230 and #312) | **mitigated (forkd: per-sandbox bearer gate, constant-time, never logged; per-sandbox concurrency cap, 429 on excess; force-close-on-terminate; standalone: loopback tokenless by design; SSRF allowlist-of-one; streaming not buffered; internet-facing edge proxy, TLS, and subdomain routing are slice 2 and NOT part of this surface)** | The expose route reverse-proxies an HTTP request (any method: GET for SSE, POST for agent RPC) to the guest's `127.0.0.1:<port>` over the existing vsock PortForward tunnel, streaming the response immediately (`FlushInterval -1`, no output buffering), so SSE sessions work without batching. Code: `internal/daemon/expose_conn.go` (net.Conn over the tunnel), `internal/daemon/expose.go` (`ProxyHTTP`), `internal/daemon/expose_route.go` (`checkBearer`, `handleExpose`, `HandleExpose`), `internal/daemon/expose_cap.go` (`acquireExpose`, `SetMaxExposePerSandbox`, `CloseExpose`, `trackExposeConn`). The surface is bounded as follows. (1) AUTH on forkd: the per-sandbox bearer token is validated by `checkBearer` with a constant-time compare (`subtle.ConstantTimeCompare`); a missing, malformed, or wrong token is rejected 401. The token VALUE is never logged and never appears in an error body; only the sandbox id and port are logged. `checkBearer` is mounted OUTSIDE the body-peeking `requireBearer` middleware so the auth gate never buffers the request body: a large or streaming POST body is not read before auth completes and is not held in memory. (2) STANDALONE: inherits the same loopback tokenless trust model as the rest of the standalone sandbox-server (`AllowTokenless`), the same as the `/forward` path; this is not a new weakening. (3) SSRF: the guest dial is forced to loopback by the guest agent (`guest/agent-rs/src/service/portforward.rs`): the host carries only a bare integer port parsed from the URL path, the guest always dials `127.0.0.1:<port>` and refuses any non-loopback target or out-of-range port; the host never derives the dial target from any user-supplied input beyond the port integer, so there is no SSRF path to steer the tunnel to another guest interface, a host interface, or the host network. Port is range-checked to 1-65535 before the tunnel is opened. (4) Each request opens its own vsock tunnel and guest TCP connection and tears it down on close; there is no multiplexing and no shared state across requests. (5) CONCURRENCY CAP: `acquireExpose(sandboxID)` (modeled exactly on `acquireStream`) enforces a per-sandbox ceiling (`defaultMaxExpose=16`, configurable via `SetMaxExposePerSandbox`) on concurrent OPEN expose tunnels; a NEW request over the cap is rejected 429 before any tunnel is dialed; existing in-flight tunnels are never killed by the cap. The counter is decremented via an idempotent `sync.Once` release deferred in `handleExpose`. This mirrors the `/forward` cap (`SetMaxForwardsPerSandbox`) and the streaming-exec cap (`SetMaxStreamsPerSandbox`). (6) FORCE-CLOSE ON TERMINATE: every expose `net.Conn` created in `ProxyHTTP`'s dial closure is registered in `exposeConns[sandboxID]` via `trackExposeConn`; `CloseExpose(sandboxID)` closes every tracked conn and clears the entry; `UnregisterSandbox` calls `CloseExpose` (before taking its own lock, mirroring `CloseForwards`) so no tunnel goroutine outlives a terminated sandbox. A `trackingConn` wrapper calls `untrackExposeConn` on natural close so the map does not grow unbounded across request lifetimes. (7) DEFERRED to slice 2 and explicitly NOT part of this surface: the internet-facing edge proxy, TLS termination, the `<label>.<expose-domain>` subdomain scheme, the Host allowlist and reserved-name check, the wildcard certificate, the controller route-sync CRD fields, and the `internal/preview` to `internal/expose` package rename. This row covers only the forkd `:9091` and standalone sandbox-server paths; the public ingress surface is the section 7c area (preview URL rows). |
...
| Per-sandbox egress proxy (networked live fork, #336) | **partial** | The per-node egress proxy (`internal/egressproxy`, raw-forkd path with `--enable-networking` and `--egress-proxy`) owns every upstream TCP connection on the host side: the guest's outbound socket is always to the proxy at the fork-stable sentinel address `169.254.169.2:3128` (DNATed per fork to the fork's gateway tap IP via nftables), never directly to the far end. This means a live-forked child cannot inherit an open upstream socket from its parent; the child re-dials through the proxy as a fresh connection after its /30 identity change. Surface: the proxy sits on outbound traffic that may carry secret-bearing headers (Authorization, API keys, tokens). Mitigation: HTTPS flows are carried via HTTP CONNECT pass-through with NO TLS interception, so the proxy never sees request headers, bodies, path, query, or auth credentials beyond the CONNECT target host:port; the proxy logs only the upstream host:port and byte counts. Each connection is attributed to a sandbox by source guest IP (each sandbox has a distinct /30 from the identity allocator); an unregistered source IP is refused with 403, so the proxy is NOT an open relay. DESTINATION CONFINEMENT (corrected): proxied egress is dialed HOST-side (source = node IP, OUTPUT path), so it is NOT subject to the per-sandbox nftables FORWARD chain (which matches on the guest source IP and carries the unconditional cloud-metadata drop); that nft block does NOT cover the proxied path. Instead the proxy enforces its OWN hard destination DENYLIST in `internal/egressproxy` BEFORE any dial, on both the CONNECT and the plain-HTTP path: `169.254.0.0/16` (covers IMDS `169.254.169.254` and ECS task metadata), the IMDSv6 address `fd00:ec2::254`, IPv4 loopback `127.0.0.0/8` (forkd's own gRPC `:9090` and sandbox API `:9091`), IPv6 loopback `::1`, and IPv6 link-local `fe80::/10`. A name target is resolved and EVERY resolved A/AAAA is screened, then a vetted resolved IP is dialed directly (DNS-rebinding safe); a denied target is refused (CONNECT 403, plain-HTTP 403) with NO dial and logged as sandbox + host:port + denied only. This denylist is what blocks IMDS credential theft and SSRF to forkd on the proxied path. ALLOWLIST GAP (residual, partially closed): the per-sandbox domain egress ALLOWLIST has two enforcement mechanisms under #494: (a) the DNS proxy feeding nftables named sets (#47, the resolve-then-pin rows above), and (b) a host-side TLS SNI peek-and-splice filter (the next row, `internal/sniproxy`) that enforces the SAME per-sandbox domain allowlist at TLS connect time for the shared-IP case. The egress proxy's OWN CONNECT/plain-HTTP forward path does NOT yet apply the domain allowlist: when `--egress-proxy` is on and a guest is configured to use the HTTP forward proxy explicitly, a registered guest may reach ANY non-denylisted destination over the proxy. The denylist is the security floor; the DNS allowlist plus the SNI filter are the name-level allowlist on the transparent datapath; an allowlist on the explicit HTTP-forward-proxy path remains a follow-up. On a live fork: the child gets a fresh /30 + NIC rebind (the eth0 re-address is the PRIMARY reset: removing the old guest IP kills any inherited socket bound to it), plus a best-effort host conntrack flush on the CHILD's fresh guest IP (`conntrack -D -s <child guest IP>`, a belt-and-suspenders that rarely matches anything because the child IP has no prior host flows; flushing the SOURCE/parent IP is deliberately AVOIDED since it would disrupt the still-running parent), and NotifyForked carries `ResetUpstreams=true` so the guest drops stale neighbor state and writes the proxy env; the child then re-dials. Engine.ForkRunning fails closed for networked sandboxes when networking is on but the proxy is not active. RESIDUAL (reviewed finding): the proxy listener binds the node wildcard on port 3128, so the port is reachable on the node's primary and pod IPs as well as the sandbox subnet. The only gate is source-IP attribution (an unregistered source is refused 403, so it is NOT an open relay), but a defense-in-depth host nftables INPUT rule limiting port 3128 to the sandbox subnet is recommended and not yet implemented. The guest is also given an advisory `NO_PROXY` (loopback + metadata) so cooperative clients bypass the proxy for those, but the host denylist is the authoritative control. A plain-HTTP (non-CONNECT) request's target path is visible to the proxy but is never logged; agents are steered to HTTPS via the baked proxy env. Status: partial (proxy + attribution + redaction + hard destination denylist + bounded preamble reads implemented and unit-tested in `internal/egressproxy`; per-sandbox allowlist on the proxied path is NOT yet enforced, tracked as #494; live end-to-end verification on real KVM is asserted by the networked-live-fork acceptance phase in this PR; the host INPUT hardening is a recommended follow-up). |
| Host-side TLS SNI egress filter (domain allowlist at connect time, #494) | **partial (peek + allowlist decision + splice implemented and unit-tested host-side; the nftables redirect that feeds it is a KVM follow-up)** | A new host-side TLS SNI peek-and-splice filter (`internal/sniproxy`, raw-forkd path with `--sni-egress`, which requires `--enable-networking`, `--enable-dns-egress`, and `--egress-proxy`) enforces the per-sandbox DOMAIN allowlist at TLS connection time. It is the SNI slice of #494 and complements the DNS resolve-then-pin rows above: the DNS proxy decides which names a sandbox may RESOLVE and pins their IPs, and this filter additionally checks the TLS SNI when the guest connects, which is the only place name-level policy can be enforced for the shared-CDN-IP case (an allowlisted and a denied name behind one IP). For each transparently-redirected connection the filter PEEKS the TLS ClientHello and extracts the SNI `server_name` WITHOUT terminating TLS (it drives the stdlib handshake parser only far enough to parse the ClientHello, then aborts before any ServerHello; the exact ClientHello bytes are buffered and replayed to the upstream on allow), so the guest's TLS session stays end to end and the filter NEVER sees a plaintext application byte. The allowlist decision REUSES the exact same matcher and per-sandbox allowlist as the DNS proxy (`dnsproxy.Registry` via `sniproxy.RegistryAllowlist`: exact or anchored-wildcard `*.D`, attributed by guest source IP, gated on the connection's port); there is NO second matcher. ALLOW splices the connection to the ORIGINAL destination (recovered host-side via `SO_ORIGINAL_DST`); DENY closes fail-closed with NO dial for a non-allowlisted SNI, a MISSING SNI (a TLS connection with no SNI is denied, so a guest cannot reach an allowlisted name by omitting it), a malformed or non-TLS first record, or a source IP with no sandbox attribution. The splice dial leaves the host OUTPUT path and so bypasses the per-sandbox nftables FORWARD metadata drop, exactly like the egress proxy, so the filter REAPPLIES the same denied-IP floor (`egressproxy.IsDeniedIP`: cloud metadata, loopback, link-local, the unspecified address, the NAT64 wrap of IMDS) before dialing; an allowlisted SNI cannot be used to reach IMDS or forkd's own ports. HONEST MODEL: this is SNI-based filtering, NOT full TLS interception (no MITM). The filter trusts that the SNI names the host the connection is actually for; it does not terminate TLS, validate the server certificate, or read any byte after the ClientHello. An attacker who controls BOTH ends can present a benign SNI and then speak to a different server (domain fronting), so SNI filtering is a policy and egress-shaping control layered on top of the DNS allowlist and the denied-IP floor (which bound which IPs are reachable at all), not a cryptographic guarantee. Status: partial. The peek (TLS 1.2 and 1.3 ClientHello), the exact and anchored-wildcard allowlist decision, the missing/malformed/non-TLS deny, the denied-IP floor, and the splice are implemented and UNIT-TESTED in `internal/sniproxy` with no VM (crafted ClientHello byte streams); forkd builds and runs the filter under `--sni-egress`. The per-sandbox nftables rule that REDIRECTS guest `tcp dport 443` to the filter and IPv6 original-destination recovery are NOT yet emitted (`internal/netconf`), so until that KVM-gated follow-up lands no traffic reaches the listener and the filter is inert. |
...
| Reuse-or-rebuild gate + artifact ownership invariants (#583, #584) | **mitigated** | `Engine.CreateTemplate` no longer unconditionally rebuilds a template from scratch on every call: an on-disk template discovered after a forkd restart, or left by a prior failed rollout, is reused ONLY when its recorded digest re-verifies (the Verify-on-load row above) AND its snapshot artifacts (`rootfs.ext4` when present, `snapshot/mem`, `snapshot/vmstate`) pass a new READ-side ownership invariant, `checkTemplateArtifactInvariants` (`internal/fork/template_invariants.go`): every artifact must be owned by the running process's effective uid and carry mode `0o644`. This closes the read side of incident #583, where a jailed build can leave template artifacts owned by the jailer's throwaway build uid instead of the daemon's euid, a state the husk VMM cannot open even though the digest itself still verifies. A template that fails either check is deleted (`Engine.DeleteTemplate`, which also unpins its CAS manifest so the chunks become GC eligible) and rebuilt in the same `CreateTemplate` call, retiring the prior hand `rm -rf templates/*` recovery. The decision is isolated in `shouldReuseTemplate` so it is unit-testable without a real Firecracker build, and covered at the Engine level (reuse, corruption-triggered rebuild, forced rebuild) without KVM via the `runTemplateBuild` seam. A new `force_rebuild` field on `CreateTemplateRequest` lets the controller skip reuse unconditionally, even over a template that would otherwise pass both checks; the controller sets it when the template's declared CONTENT changed (issue #475), since digest verification proves integrity, not content identity. The field carries no new authentication surface: it rides the same mTLS-gated, controller-only `CreateTemplate` RPC as the rest of the request (Publish authorization row above). Residual: the write-side normalization that keeps a freshly built template's ownership correct (`normalizeTemplateArtifacts`) is tracked separately as #587; this row is the read-side refusal only. |
| Compatibility verification (no unsafe restore) | **mitigated** | The same load gate also runs the snapshot compatibility contract (`internal/snapcompat.Check`, issue #32) after the digest verify and before any Firecracker launch. The manifest records the producing environment (snapshot format version, Firecracker version, CPU model, kernel, config hash, and the guest-agent vsock protocol version) as part of the content-addressed digest, so these fields cannot be tampered with or downgraded without changing the digest and failing the verify-on-load step above. A benign mismatch (a snapshot legitimately built under a different Firecracker version, a different CPU model, an unsupported format version, or a guest agent speaking a protocol this build cannot drive) fails closed: the restore is refused with an actionable error rather than crashing or silently corrupting a guest. The guest-agent protocol gate (issue #459) closes a specific upgrade hazard: a pure runtime upgrade can leave format/Firecracker/CPU all matching while the baked agent speaks an older protocol, which previously surfaced only as an opaque vsock `BrokenPipe` at the fork-correctness handshake; it is now refused at load with a rebuild message. The dev-mode escape `--allow-incompatible-snapshots` downgrades a refusal to a loud warning. Kernel mismatch is informational. Residual: cross-CPU-model restore via Firecracker CPU templates and live cross-Firecracker-version restore are out of scope (the contract refuses them today). |
...
| Scoped API keys: least-privilege blast radius (issue #784) | partial | A key is minted with SCOPES (`read`, `execute`, `lifecycle`, `admin`) that shrink its reach below the org, so a browser-safe or CI-safe key can do strictly less than the whole org: `read` lists, gets, and statuses (inspecting an existing sandbox needs no stronger scope), `execute` acts inside an existing sandbox (exec/files/run_code) but cannot create or destroy, `lifecycle` creates/forks/terminates (and pause/resume) but cannot exec, and `admin` is reserved for org management (keys/billing) orthogonally to the resource scopes. The gateway authz path maps each op to its required scope (`requiredScopeFor`, `internal/saas/gateway.go`) and DENIES a key that lacks it with an LLM-legible `forbidden` (403) whose remediation names the exact scope and whose context reports `op` and `required_scope`; an unmapped op fails CLOSED to `lifecycle` (the most privileged resource scope) so a newly added route cannot slip through on a lower scope. The scope model applies a one-way implication graph (`ApiKey.HasScope`/`scopeSatisfies`, `internal/saas/model.go`): every resource scope implies `read` (no dead end), `execute` and `lifecycle` are orthogonal, `admin` implies and is implied by nothing, and the LEGACY `sandboxes` scope satisfies read/execute/lifecycle so no existing resource key is broken. FAIL-CLOSED is enforced at both ends: a key stored with NO scopes satisfies NOTHING (`ApiKey.HasScope` iterates the held scopes with no empty-set special case; `TestScopelessKeyFailsClosed`, `TestGatewayScopelessKeyIsDeniedEverywhere`), because there are no genuine pre-scopes records to preserve (the `api_keys.scopes` column is `NOT NULL DEFAULT '{}'` from the first migration and enforcement shipped with the api-key feature), so an empty scope set is an inert key rather than a legacy full-access one, and promoting it to full access would silently escalate every empty-scope key the console can mint to admin. Minting is also fail-safe: a key requested without scopes defaults to the NON-ADMIN resource set (`CreateKey` -> `DefaultScopes()` = read/execute/lifecycle, matching the CLI default; `TestCreateKeyDefaultsToNonAdminScopes`), never the admin-inclusive `FullScopes()`, because minting is a routine member action that must not hand out a management credential; `admin` is granted only when requested by name (`TestCreateKeyAdminRequiresExplicitRequest`). A scope outside the closed vocabulary is refused at mint (`ErrUnknownScope`). Unit-tested end to end at both the key layer (`internal/saas/scopes_test.go`) and the gateway (`internal/saas/gateway_scopes_test.go`: read denied every mutation, execute can exec but not create, lifecycle can create but not exec). The optional PER-RESOURCE qualifier (a scope narrowed to a named sandbox or pool) is a documented follow-up, NOT this change; there is no API-key-reachable gateway admin operation yet (key/billing management runs through the session-authenticated console), so the `admin` scope is minted and persisted today (via the API and the CLI `--scopes`) and gates that surface when it lands. RESIDUAL: the console key-create form still exposes only the older `sandboxes`/`read` vocabulary, so the finer scopes cannot yet be selected in the UI where the 403 remediation text points; minting a finer-scoped key currently goes through the CLI or API. That console-form gap is a tracked follow-up, not a backend authz weakness. |
...
| Single-tenant-mode referenced-object isolation (`secretRef`, `workspace`; GHSA-pgv2-9w24-j7wh) | **mitigated (create refuses referenced objects in single-tenant mode; controller resolvers refuse cross-org labels)** | Single-tenant mode (`WithSingleTenantNamespace` / helm `singleTenantNamespace`) pins every org's sandboxes into ONE shared namespace. Sandbox LIFECYCLE authz still holds (the `getOwned` org-label re-check, the cross-org row above), but an object a create REFERENCES by bare name has no org boundary in a shared namespace: a `secrets[].secretRef.name` or a `workspace` name is resolved by the controller with a bare `Get` in the sandbox namespace, so a tenant could name a PLATFORM Secret (the database DSN, mail or registry credentials that are co-located in the shared namespace) or another tenant's Secret/Workspace and have its value mounted into its own sandbox and read via `env`. The controller's create-failure reason (`secret X not found` vs `key Y not found in secret X`) also enumerated which objects exist. FIX: the gateway create path REFUSES any `secretRef` or `workspace` reference while single-tenant mode is set (`internal/saas/controlplane` create, `TestCreateRejectsSecretRefInSingleTenant`/`TestCreateRejectsWorkspaceRefInSingleTenant`), so the client path to a shared-namespace object is closed; and as defense in depth the controller resolvers refuse a referenced Secret/Workspace whose `mitos.run/org` label differs from the claim's org (`resolveSecrets`/`resolveWorkspaceHead`, `TestResolveSecretsRefusesCrossOrgLabeledSecret`/`TestResolveWorkspaceHeadRefusesCrossOrg`), which blocks a cross-org labeled reference even on a non-gateway create path. Per-org namespace tenancy (section 7a) is the production posture and lifts the create restriction, because the namespace then IS the boundary. Residual: co-locating platform secrets in a tenant namespace remains an operator misconfiguration to avoid (move them to a control-plane-only namespace); single-tenant mode stays a single-CUSTOMER QA posture, not a multi-tenant one. |
| Hosted live fork op (`sandbox.fork`, issue #709) | partial | `POST /v1/sandboxes/{id}/fork` routes to the dedicated `sandbox.fork` op, served by the control plane as a live fork: the SOURCE is resolved org-scoped from the path (`getOwned`: the org namespace AND the `mitos.run/org` label re-check, where the `OrgID` comes ONLY from the verified key), so a key for org A naming org B's sandbox id gets `not_found`, indistinguishable from a missing id, and NEVER forks it (`TestForkCrossOrgSourceIsNotFoundAndNeverForks`); id-guessing across orgs therefore yields 404s, not forks. A missing source fails fast (the #646 precedent) and a not-Ready source is a 409 with remediation, both before any object is created. The submitted Sandbox carries `source.fromSandbox` naming the source and NO `poolRef`, in the caller's org namespace with the org label, so the controller's fork engine (#611) enforces its own gates on top: the never-widen fork budget and the secret-inheritance default-deny (a generic `Rejected` condition surfaces the controller's message as a 409, never a silent timeout; the `SecretInheritanceDenied` reason surfaces as a 403 whose remediation names the exact wire-level opt-in, `"secret_inheritance": "inherit"`). The body's `secret_inheritance` field is the EXPLICIT opt-in for duplicating the source's in-memory secrets into the child: it is validated against the two modes (`reissue`, the default; `inherit`), carried over the authenticated wire onto `spec.secretInheritance`, and audited by the controller's `SecretInheritance/ExplicitOptIn` condition; `pause_source` freezes only the caller's own org-owned source during the checkpoint. A caller-named child id is DNS-1123 pre-validated (typed 400) before any object is created. The child's token is REISSUED per fork (the source's token never opens the child); the create-shaped response returns the child's endpoint and token exactly once. RUNTIME HARDENING (adversarial review of #710): a reaped fork CHILD answers runtime calls with the documented typed idle_timeout/terminal error BEFORE any dial (the child phase is consulted, not just the parent fork object's, which stays Ready while the reaper flips the child; `TestForkChildTerminatedYieldsTypedErrorNotProxied`), and a multi-child fan-out created outside the gateway (cluster SDK, kubectl) is refused typed on the runtime surface rather than silently routed to child 0 (`TestForkMultiChildRuntimeIsTypedErrorNotChildZero`). QUOTA: `sandbox.fork` is classed as a create in the enforcer (`quota.Request.isCreate`), so the live concurrency cap and the creation-rate bucket apply and a capped org cannot fan out past its tier via forks (`TestForkOpCountsAsCreateForCaps`); the `GatewayAdapter.SizeOf` seam now applies to `sandbox.fork` exactly as to `sandbox.create` (`TestForkOpSizeOfAppliesThroughAdapter`), though `SizeOf` remains unwired in the shipped gateway, so fork size is checked with a zero spec today, the same honest limit as create. BILLING AND REAP: fork child pods are stamped with the `mitos.run/claim` label at build time (`buildForkChildPod`), so the usage scraper (`HuskPodScrapeLister`) meters them under the hosted fork id and the claim-label pod-deletion paths (release, lifetime terminate) reap them; before this a fork child carried only husk/fork/org labels and was silently unbilled. A successful fork emits the non-PII `sandbox.forked` product event (salted org hash, success flag, pool name only). Residual: the route serves ONE child per call (`replicas > 1` is a typed `invalid_input`); a source that is ITSELF a fromSandbox fork is rejected typed (`TestForkOfForkIsRejected`) until the controller fork-of-fork resolution is proven on this surface (the running VM is the fork's child, not the fork object a new FromSandbox would name); the SDK's `Idempotency-Key` is not yet honored by the hosted control plane (same standing gap as create), so a retried fork can double-create; and fork fan-out remains bounded by the source's node by construction (a live fork copies already-resident guest memory in place), so a single sandbox cannot fan out past its node's capacity through this route. |
| Gateway WebSocket runtime proxy (interactive PTY) | partial | The interactive PTY rides the Connect `sandbox.v1.Sandbox.Exec` RPC over a WebSocket, a connection UPGRADE the buffer-and-stream `Forward` seam cannot carry, so the gateway proxies it directly (`internal/saas/gateway_ws.go`): it detects the upgrade on the runtime op, resolves the org-scoped endpoint and per-sandbox token through the `RuntimeResolver` seam (`internal/saas/controlplane` `ResolveRuntime`), and reverse-proxies with the Upgrade preserved so `httputil.ReverseProxy` hijacks the connection and pipes the WebSocket bidirectionally. AUTH, scope, and quota are enforced BEFORE the upgrade by the SAME gateway pipeline as every runtime op (a non-upgrade request is unchanged). The `OrgID` is taken SOLELY from the verified key; a missing or cross-org sandbox id collapses to `not_found` and is NEVER dialed (`TestResolveRuntimeCrossOrgIsNotFound`, `TestGatewayWebSocketCrossOrgNotFound`), so the cross-org invariant above is unchanged. The customer key is NEVER forwarded to the sandbox: the Director overwrites Authorization with the per-sandbox token read from the `<name>-sandbox-token` Secret, asserted in `TestGatewayWebSocketProxyEndToEnd` (the backend sees the per-sandbox token, never the customer key); the token value is never logged. The sandbox id travels on the `?sandbox=` query the daemon ws Exec endpoint reads (`internal/daemon/exec_ws.go` `ptyAuth`). Residual: the per-sandbox token crosses the in-cluster hop to forkd/husk `:9091` in CLEARTEXT, the SAME trust model as the HTTP runtime proxy and the expose path (in-cluster TLS is the recorded follow-up); and the warm-husk path requires the husk-stub image to carry the #390 single-sandbox Connect resolution, else `ptyAuth` / `connectLookupToken` rejects the gateway-attached token 401, so a stale dormant husk-stub image must be recycled to the current tag. |
...
| Kill-switch / org suspension (issue #36) | partial | Org suspension is a first-class verb (`internal/saas/quota` `KillSwitch`): suspend one org (manual review hook), suspend a set at once (pool-wide / org-wide emergency stop), or suspend automatically from an abuse signal. A suspended org fails closed: the enforcer denies it before any quota math (`forbidden`), so its keys are rejected at the gateway and new claims are refused. An automated ABUSE or emergency-stop suspension is held for human review before it can be lifted; automated BILLING suspensions (spend cap, dunning) carry no hold and lift on the matching payment event via the reason-scoped `LiftReason`, which never touches another reason or any held suspension. Unit-tested: a suspended org's request is rejected, lifting restores access, emergency stop suspends all named orgs, an abuse signal drives suspension. The VM-freeze effect on already-running sandboxes and the durable suspension store are documented seams. |
| Metered billing and payment secrets (issue #212) | partial | The billing core (`internal/saas/billing`, docs/saas/pricing.md) wires Stripe metered usage, a credit ledger, spend caps, dunning, and a webhook handler. It is built ENTIRELY against a `StripeClient` interface with an in-memory `FakeStripe`: NOTHING in this slice makes a real charge, and the Stripe Go SDK is not a hard dependency. A Stripe API key, the webhook signing secret, and any payment-method detail are SECRETS that are never logged, never put in an error or a condition/webhook note, and never written to a host path; the kill-switch and webhook notes carry org ids, caps, and counts only. The metered push is idempotent on the `(org, sandbox, window)`+meter key (the same #211 record key), so a retried push never double-reports; the credit ledger is append-only and floors at zero so accounting cannot go negative or double-debit. DRAWDOWN ACCUMULATOR (issue #662): usage is priced in MILLI-cents and folded into a per-org signed sub-cent remainder (`drawdown_remainders`, migration 0010; in-memory mirror in `MemCreditLedger`), settling whole cents into the cents-based ledger with round-half-up, so a steady small sandbox is billable (per-record cent rounding previously settled zero forever) and cumulative debits track cumulative usage within half a cent at all times. CONSISTENCY POSTURE: the ledger debit, its `(org, sandbox, window)` idempotency key, and the new remainder commit in ONE atomic ledger step (`AppendWithRemainder`: a single Postgres transaction in `PgCreditLedger`, one mutex hold in memory); a replayed window hits the duplicate key BEFORE any state moves, so a replay can neither double-debit nor double-count the carry, and no crash can land a debit without its remainder. Residual: concurrent drawdown drivers settling the SAME org interleaving the remainder read and atomic write would lose a sub-cent carry (last write wins, never a double debit); the console runs one sequential driver so this does not occur in the shipped wiring. The drawdown log line carries the cycle's AGGREGATE settled cents and carried milli-cents (issue #665) so a zero-settling system is visible; it still never logs a per-org balance or any secret. A breached HARD spend cap suspends the org through the #213 kill-switch (the runaway-agent backstop) WITHOUT a manual hold: a cleared paid top-up is the automated lift lever (reason-scoped `LiftReason`; the spend window resets at the payment), and manual holds remain operator-only and survive every automated lift. Unit-tested: idempotent push (incl. retry after a transient failure), credit drawdown/no-negative/idempotent replay, soft-cap alert vs hard-cap suspend, dunning transitions, webhook status update. SEAMS (documented follow-ups, NOT enabled here): the real Stripe SDK adapter and the real Stripe webhook SIGNATURE verification (the signing-secret check over `Stripe-Signature`; the test uses a fake verifier that trusts the body). DURABLE STORES (issue #614): the money-relevant billing state is Postgres-backed when the console has a DSN configured; the credit ledger (`pgstore.PgCreditLedger`), spend caps (`pgstore.PgSpendCapStore`), the per-org billing status / dunning state (`pgstore.PgStatusStore`), and the org-to-provider-customer map (`pgstore.PgCustomers`) all survive a console restart, so a `past_due`/`suspended` org set by a billing webhook cannot silently revert to active and a webhook arriving after a redeploy can still map its customer id back to the org (a paid top-up is not orphaned). The webhook handler answers a customer-store LOOKUP FAILURE with 5xx (never a 2xx ack), so the provider retries and a transient store error cannot permanently drop a status sync; an unknown customer is still acked. CUSTOMER-LINK WRITE (issue #618): the webhook is also the writer of the org-to-customer map; when an event's SIGNATURE-VERIFIED custom_data names the org and the envelope names the customer, the handler records the link before processing, so the first transaction for a new customer maps every later custom-data-less event (subscription.canceled etc.) back to the org. The link is written only after signature verification (a forged body cannot repoint a customer to an attacker org), is idempotent last-write-wins, and a link-store FAILURE is a 5xx retry, never a silent drop. A best-effort checkout-time link from the create-transaction response never fails the checkout; its failure log carries the org id only, never a key or the checkout URL. The in-memory stores remain the DEV-ONLY fallback when no DSN is set. The webhook HTTP entry point never echoes the raw body or the signature in an error. PRODUCTION GATE: real-charge wiring is not enabled and must be reviewed (key handling, no-secret-logging, no-unverified published prices) before going live. |
...
| Console org audit trail durability (issue #616) | **mitigated (durable when Postgres is configured); open (retention enforcement)** | The org audit trail the console records (key create/revoke, member changes, billing actions) and serves at `GET /console/audit` plus the NDJSON export is now DURABLE when the console has a database DSN: `cmd/console` wires `pgstore.PgAuditLog` (migration `0009_audit_log.sql`, append-only `audit_log` table) behind the same `console.AuditRecorder` seam, mirroring the session/spend-cap/allowlist store selection; without a DSN the in-memory `MemAuditLog` remains the dev fallback with a loud startup warning. The store is APPEND-ONLY: no UPDATE path exists, and the only permitted delete is retention pruning by age. `List` is org-scoped by a parameterized `WHERE org_id` from the verified caller context, so the baseline cross-org isolation invariant is unchanged (tested: org A never sees org B rows, plus restart survival across a second `pgstore.Open`, in `internal/saas/pgstore/auditlog_test.go`). Events carry identifiers and a non-secret detail string only, never a key value or any secret (the `AuditRecorder` contract), so the table stores no secret material. Residual: retention ENFORCEMENT (the pruning sweep applying the per-org retention days the console stores) does not exist yet for either store; it is the controller GC follow-up (issue #163), so an org's trail currently grows unbounded until that lands. |
| Console BFF per-verb permission enforcement (B3d-1) | **mitigated** | Every mutating console endpoint is gated by a permission verb from the enforced vocabulary (`members.manage`, `projects.manage`, `secrets.manage`, `settings.manage`, `billing.manage`, `resources.use`, `read`). The gate is DENY BY DEFAULT: `Console.authorize` (`internal/saas/console/authz.go`) resolves the caller's role via `permissionsFor`, and if the required permission is absent the handler returns 403 with a cause naming the missing permission. An unknown role string (one that matches no built-in and no custom role in the org store) returns an empty permission map, so it is denied on every verb without a separate error path. The org is sourced from the verified session/key context only and is never taken from the request body or path, so the existing cross-org isolation invariant is unchanged. Built-in roles (`owner`, `admin`, `member`, `viewer`, `billing`) are resolved through `saas.Role.Can` over the fixed `knownPermissions` slice; a custom role's permissions are looked up in the `CustomRoleStore` scoped to the caller's org. Unit-tested: `internal/saas/console/authz_enforce_test.go` covers deny-by-default for unknown roles, per-verb allow/deny for each built-in and for custom roles, and the 403 error shape. |
| Console BFF custom-role management restricted to settings.manage (B3d-1) | **mitigated** | `POST /console/roles` (upsert) and `DELETE /console/roles/{name}` (delete) are gated by `PermManageSettings`. Only built-in roles that carry `settings.manage` (Owner and Admin) can create or modify custom roles; Member, Viewer, and Billing cannot. A caller without `settings.manage` receives 403 before the body is decoded. `GET /console/roles` is gated by `PermReadOnly` so any org member can read the permission matrix. This is enforced in `handleUpsertRole` and `handleDeleteRole` (`internal/saas/console/customroles.go`). |
| Console BFF custom-role vocabulary invariant (B3d-1) | **mitigated** | A custom role submitted to `POST /console/roles` is validated at write time: every permission in `role.Permissions` must appear in `knownPermissions`; any unknown string is rejected with `invalid_json` before the role is stored. A custom role cannot shadow a built-in role name (`owner`, `admin`, `member`, `viewer`, `billing`): the handler checks `builtinRoles[role.Name]` and rejects a collision. These two checks are enforced in `handleUpsertRole` and unit-tested in `internal/saas/console/customroles_test.go`. |
| Console BFF custom-role org isolation (B3d-1) | **mitigated (unchanged from the baseline cross-org row)** | Custom roles are stored and resolved in the `CustomRoleStore` keyed by `orgID`, which is sourced from the verified caller context only (never from the request body or path). `MemCustomRoleStore` uses a `byOrg` map and every read/write takes the orgID from the context; a write for org A can never affect org B, and a read for org A can never return org B's roles. This is the same isolation invariant as the baseline cross-org row; B3d-1 does not widen or narrow it. |
| Console BFF per-project sandbox access: list filtered to accessible sandboxes (B3d-4) | **mitigated** | `GET /console/sandboxes` applies `canAccessSandbox` to every sandbox before including it in the response. The gate is DENY BY DEFAULT: if the caller holds no applicable permission (no org-wide role, no project membership), the sandbox is silently excluded from the list rather than returned. The decision never falls through to an allow: a caller with an empty permission map (unknown role, no membership) sees an empty list. The org for both the org-wide and the project membership lookups is taken from the verified session context, never from the request, so the filter cannot be bypassed by supplying a different org id. Unassigned sandboxes apply org-wide permissions (any org member with `read` can see them). Assigned sandboxes are restricted to org managers (`projects.manage`) plus members of that specific project. Intentional behavior change: assigning a sandbox to a project RESTRICTS its visibility; only project members and org managers can see it in the list. Implemented in `internal/saas/console/console.go` (`handleListSandboxes`); the access matrix is exhaustively tested in `internal/saas/console/projectaccess_test.go`. |
| Console BFF per-project sandbox access: inspect of an inaccessible sandbox returns 404 (B3d-4) | **mitigated** | `GET /console/sandboxes/{id}` resolves the sandbox, then calls `canAccessSandbox`. If the caller cannot access the sandbox, the handler returns 404 (not 403). This avoids leaking the existence of the sandbox to a caller who has no right to know it is there: a caller that has no access to a project-assigned sandbox receives the same response they would receive for a nonexistent id. Implemented in `internal/saas/console/console.go` (`handleInspectSandbox`); tested by the no-access-to-project rows of the access-matrix test. |
| Console BFF per-project sandbox access: terminate of an inaccessible sandbox returns 403 (B3d-4) | **mitigated** | `DELETE /console/sandboxes/{id}` resolves the sandbox, then calls `canAccessSandbox` for `PermUseResources`. If the caller cannot access the sandbox, the handler returns 403. Terminate is a destructive action, so the gate is different from inspect: the caller learns they are denied the action rather than that the resource does not exist (this is consistent: the caller either already knows the id from a prior list they could see, or is guessing, in which case 403 on an accessible resource and 403 on an inaccessible one are indistinguishable). Implemented in `internal/saas/console/console.go` (`handleTerminateSandbox`); tested by the no-access-to-project terminate rows of the access-matrix test. |
| Console BFF per-project sandbox access: org-scoped decision, no cross-org widening (B3d-4) | **mitigated (unchanged from the baseline cross-org row)** | `canAccessSandbox` (`internal/saas/console/projectaccess.go`) resolves both the org-wide permission set (`permissionsFor`) and the per-project membership set (`projectPermissionsFor`) using the orgID from the verified session context. Neither lookup accepts an org id from the request body or path. A project-level membership in org A cannot grant access to a sandbox in org B, because the project membership list is always fetched for the caller's org. This preserves the baseline cross-org isolation invariant unchanged through the per-project access layer. |
| Gateway/console metrics listeners (issue #617) | partial | The gateway and console each serve Prometheus metrics on a SEPARATE cluster-internal listener (`--metrics-addr`, default `:9100`), NEVER on the public mux, so an internet client cannot reach `/metrics` through the public Service or Ingress (the chart exposes it only as a named container port scraped by a PodMonitor). The exported series are aggregate counts, bounded label classes (status class, denial reason), and timestamps ONLY: no key material, org id, customer ref, path, balance, cost, or error detail ever enters a label or value (the drawdown and webhook metrics are deliberately label-free; the per-org usage gauges live on the controller, unchanged). Residual: the listener is plaintext HTTP and unauthenticated INSIDE the cluster, the same trust model as the controller and forkd metrics endpoints; a pod on the cluster network can read aggregate traffic/denial rates (coarse operational telemetry, no tenant data). In-cluster TLS/scrape-auth remains the fleet-wide follow-up recorded for the other metrics endpoints. |
| Console live log-stream transport (`GET /console/sandboxes/{id}/logs` and `.../logs/stream`, issue #715) | **mitigated (husk-pod path); open (raw-forkd path has no transport yet)** | The console can now tail a sandbox's own stdout/stderr: a plain one-shot GET (`handleSandboxLogs`, `internal/saas/console/logs.go`) and an SSE follow (`handleSandboxLogsStream`, `internal/saas/console/sandbox_ops.go`), both backed by the `console.LogStreamer` seam (`internal/saas/console/seams.go`); in production `deps.Logs` is `clustersandbox.Control.StreamLogs` directly (`cmd/console/sandboxcontrol.go`), NOT the `AuthorizingLogStreamer` wrapper, which is only the in-memory default `console.New` falls back to when no real transport is wired. WHAT IT READS: on the husk-pod path `clustersandbox.Control.StreamLogs` (`internal/saas/console/clustersandbox/logs.go`) opens the Kubernetes pod-logs subresource stream for the sandbox's OWN husk pod (`sb.Status.Pod`) with `Follow: true`, the same source `cmd/kubectl-mitos/logs.go` already reads one-shot for the operator "logs" verb; it is READ-ONLY pod stdout/stderr, nothing else. AUTHORIZATION: both HTTP handlers resolve the sandbox via `c.deps.Sandboxes.Get` (org-scoped; a cross-org or missing id is a plain 404 with no content) and THEN call `canAccessSandbox` with `PermReadOnly` on the sandbox's resolved project tag, mirroring `handleInspectSandbox` exactly, including its 404-on-deny existence-hiding semantics: a caller who lacks per-project read access gets the SAME 404 as a nonexistent id, never 403. Only once this handler-level gate passes is `c.deps.Logs.StreamLogs` invoked, so the per-project `read` RBAC check (section 7b rows above) now genuinely applies to both log routes, not just the org-scoping check `clustersandbox.Control.StreamLogs`'s own `s.get` lookup performs underneath it. `TestProjectAccessMemberLogsSPReturns404` and `TestProjectAccessMemberLogsStreamSPReturns404` assert an org member without project access to a project-assigned sandbox is refused on both routes; `TestProjectAccessPViewerLogsSPReturns200` and `TestProjectAccessPViewerLogsStreamSPReturns200` assert a member WITH project access still streams (`internal/saas/console/projectaccess_test.go`). `TestStreamLogsCrossOrgIsNotFoundAndNeverTouchesPodLogs` and `TestSandboxLogsStreamRefusesCrossOrg` assert the pod-log transport is never reached for a cross-org id (`internal/saas/console/clustersandbox/logs_test.go`, `internal/saas/console/sandbox_ops_test.go`). WHAT IT DOES NOT EXPOSE: this surface carries no exec (no command execution, no shell, no stdin), no file read/write, and no path beyond the sandbox's own husk pod name resolved server-side; there is no cross-org read (tested above) and no way to address another sandbox's or another org's pod. BOUNDED MEMORY: `streamPodLines` reads through a bufio.Reader capped at `maxLogLineBytes` (1 MiB) per line and pumps lines through a fixed `logLineBufferCap` (512) drop-oldest channel, so neither an absurdly long line nor a slow/stalled client (SSE heartbeat keeps the connection alive, `internal/saas/console/sandbox_ops.go`) can grow console memory without bound; a line over the cap is replaced by a `[line truncated]` marker and the follow continues rather than hard-erroring the whole connection (`TestStreamPodLinesOversizedLineEmitsTruncatedMarkerAndContinues`), and a full buffer drops the OLDEST queued line first (`TestPushDropOldestDropsOldestWhenFull`). CONNECTION LIFECYCLE: the SSE handler runs the upstream follow on a background goroutine so a real, blocking pod-log follow cannot starve the heartbeat loop; on every return path (client disconnect, upstream completion, or a failed heartbeat write on a dead connection) the handler cancels a context derived from the request before returning and waits for the upstream goroutine to actually finish, so it can never call `sink.Write` after `ServeHTTP` has returned (`TestSandboxLogsStreamNoWriteAfterHandlerReturnsOnHeartbeatFailure`, `TestSandboxLogsStreamHeartbeatsDuringBlockingFollowTransport`). THE RAW-FORKD GAP: a sandbox on the raw-forkd path (no husk pod; see api/v1 `SandboxStatus.Pod`'s doc) has no log source wired yet, so `StreamLogs` reports `console.ErrUnsupported` honestly, which both routes map to HTTP 501, rather than fabricating a permanently-empty stream that would look like a quiet, healthy sandbox (`TestStreamLogsNoHuskPodIsUnsupported`, `NewUnsupportedRawLogStreamer`). Closing that gap (a raw-forkd log transport) is a documented follow-up, not a security weakening: today it fails closed and honest. |
...
| Single-use HMAC grant token | mitigated (unit-tested); HA caveat | The central-auth origin issues a short-lived HMAC-SHA256 grant token (payload + detached tag, domain-tagged as `mitos-expose-grant-v1\x00`) that is accepted exactly once: the nonce is recorded in a per-process sync.Map on first verify and rejected on replay. Expiry is checked before nonce recording. The grant token VALUE is a bearer credential: never logged, never in an error string. `internal/preview/grant.go`. Unit-tested: round-trip, expiry, replay (nonce reuse), label mismatch, tampered payload, wrong key, malformed token. HA CAVEAT: the nonce cache is per-process (in-memory `sync.Map`). With more than one proxy replica a grant presented to a different replica than the one that issued it is not yet in that replica's nonce cache, so replay is NOT rejected across replicas. Mitigation until a shared store lands: keep `expose.replicas: 1` or use sticky sessions at the load balancer so the callback hits the same replica that issued the grant. A shared nonce store (Redis/etcd) is a documented follow-up gated behind #194 before this surface serves untrusted tenants at scale. |
...
| Chromium pinned to the current security build (150) | resolved, pin retained | `images/computer-use/Dockerfile` pins `chromium=150.0.7871.114-1~deb13u1` (debian-security) and `apt-mark hold`s it. History: the first 150 security rebuild (`150.0.7871.46-1~deb13u1`) SIGTRAPed on launch (TRAP_BRKPT from Chromium's own `IMMEDIATE_CRASH`) and never bound the CDP port, so the image temporarily pinned back to `147.0.7727.137-1~deb13u1` (#723, #724), accepting a skipped security update bounded by the `--no-sandbox`-inside-microVM row above. Debian then removed 147 from the archive and the later `150.0.7871.114-1~deb13u1` rebuild launches clean, so the pin moved forward and the accepted-downgrade risk is closed; the posture is back to tracking debian-security. The exact-version pin plus hold stays deliberately: a respin can only enter via a deliberate bump, and the CI CDP smoke (`docker-build`) proves every pinned image launches, dumping filtered Chromium logs on failure so an unlaunchable package is caught in CI, not in production. |
