...
| Guest agent - Go (`guest/agent`) | PID 1 in guest, untrusted post-exec | nothing | forkd / husk stub treat its output as data only |
| Guest agent - Rust (`guest/agent-rs`) | PID 1 in guest when baked via `AGENT_IMPL=rust`; opt-in only; the Go agent is the production default. Serves ONLY gRPC on vsock port 53 (AgentGRPCPort). SECURITY-SENSITIVE: requires named human reviewer; see `docs/security-review-policy.md`. | nothing | forkd / husk stub (gRPC path only; JSON path on port 52 is NOT served) |
...
| Guest agent as attack surface (Go, `guest/agent`) | **partial** | Agent speaks JSON on vsock port 52 and gRPC on vsock port 53 (`guest/agent/main.go`); host side treats responses as data. A 10MB line-buffer cap exists. No fuzzing of the protocol yet. SECURITY-SENSITIVE: requires named human reviewer before any PR touching `guest/agent` is merged. |
| Guest agent as attack surface (Rust, `guest/agent-rs`) | **partial (gRPC surface only; opt-in)** | The Rust agent serves only gRPC on vsock port 53. It is NOT the production default; it is opt-in via `AGENT_IMPL=rust` in `guest/rootfs/build.sh`. When baked as `/init`, the JSON protocol on port 52 is not served: the production fork/exec/file host callers (`internal/fork/uffd_engine.go`, `internal/daemon/sandbox_api.go`, `internal/firecracker/template.go`, `internal/husk/stub.go`, and smoke binaries) will fail to connect until the JSON->gRPC host-caller migration (SP1.5) lands. No fuzzing of the gRPC surface yet. SECURITY-SENSITIVE: requires named human reviewer before any PR touching `guest/agent-rs/src/sys/`, `guest/agent-rs/src/fork/`, `guest/agent-rs/src/init/mod.rs`, or `guest/agent-rs/src/main.rs` is merged. See `hack/rust-agent-cutover.md`. |
