...
- Child-side memfd import (milestone m5): the co-located child boots its guest RAM
  ...
  `MAP_PRIVATE` map and FROZEN overlay `fork.ComposeChildFromImport` performs, and
  loads NO disk mem file, so with `LiveCowChildImport` on the co-located child boots
  its guest RAM from the source memfd end to end. It is gated by `LiveCowChildImport`
