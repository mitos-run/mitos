...
func validatePrepareFlags(prepareEgressLink, prepareRestore, prepareKernelPrefault, multiVMFork, enableHuskPods bool) error {
	...
	// The kernel prefault warms a guest the dormant restore already resumed, so it
	// requires the restore, and transitively the whole chain.
	if prepareKernelPrefault && !prepareRestore {
		return fmt.Errorf("--husk-prepare-kernel-prefault requires --husk-prepare-restore: there is no running guest to warm without the dormant restore")
	}
	return nil
}
...
func main() {
	...
	var prepareKernelPrefault bool
	...
	flag.BoolVar(&prepareKernelPrefault, "husk-prepare-kernel-prefault", false, "EXPERIMENTAL, DEFAULT OFF: pre-restored dormant husk pods run the template build's inert warm cell ONCE at prepare, so the run_code kernel's working set is resident before the first tenant cell instead of being demand-faulted in. REQUIRES --husk-prepare-restore. FAILS OPEN in the pod: a template without the kernel (a non-python pool) logs and continues. Off keeps the first run_code lazy byte-for-byte.")
	...
	flag.StringVar(&orgPoolName, "org-pool-name", "", "Name the cloned pool is created under in each org namespace; MUST match the pool name a create resolves for the org, i.e. the SDK's image/pool argument AND the gateway's --default-pool. If they disagree, a create resolves a pool name that has no per-org clone and the org gets a silent Pending dead end. Empty defaults to --org-pool-template-name. Only used with --enable-org-tenancy.")
	...
	if err := validatePrepareFlags(prepareEgressLink, prepareRestore, prepareKernelPrefault, multiVMFork, enableHuskPods); err != nil {
	...
	if err := (&controller.SandboxPoolReconciler{
		...
		PrepareKernelPrefault:     prepareKernelPrefault,
