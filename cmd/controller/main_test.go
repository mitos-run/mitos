...
func TestValidatePrepareFlags(t *testing.T) {
	cases := []struct {
		name                                                  string
		egress, restore, prefault, multiVM, huskPods, wantErr bool
	}{
		{"off is fine", false, false, false, false, false, false},
		{"egress with multivm and husk pods", true, false, false, true, true, false},
		{"egress without multivm", true, false, false, false, true, true},
		{"egress without husk pods (raw-forkd path)", true, false, false, true, false, true},
		{"restore with egress, multivm, husk pods", true, true, false, true, true, false},
		{"restore without egress", false, true, false, true, true, true},
		{"restore with egress but no multivm", true, true, false, false, true, true},
		{"prefault with the full chain", true, true, true, true, true, false},
		{"prefault without restore", true, false, true, true, true, true},
		{"prefault alone", false, false, true, false, false, true},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			err := validatePrepareFlags(tc.egress, tc.restore, tc.prefault, tc.multiVM, tc.huskPods)
			if (err != nil) != tc.wantErr {
				t.Errorf("validatePrepareFlags(egress=%v,restore=%v,prefault=%v,multiVM=%v,huskPods=%v) err=%v, wantErr=%v", tc.egress, tc.restore, tc.prefault, tc.multiVM, tc.huskPods, err, tc.wantErr)
			}
		})
	}
}
